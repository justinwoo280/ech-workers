...
func Init(addr string) {
	...
	go func() {
		log.Info("[BrowserDialer] Starting on %s", addr)
		err := http.ListenAndServe(addr, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.URL.Path == "/websocket" {
				if r.URL.Query().Get("token") == csrfToken {
					if conn, err := upgrader.Upgrade(w, r, nil); err == nil {
						conns <- conn
						log.V("[BrowserDialer] New browser connection established")
					} else {
						log.Warn("[BrowserDialer] WebSocket upgrade failed: %v", err)
					}
				} else {
					log.Warn("[BrowserDialer] Invalid token")
		...
		if err != nil {
			log.Error("[BrowserDialer] Server failed: %v", err)
		}
	}()
}
...
func CheckOK(conn *websocket.Conn) error {
	if _, p, err := conn.ReadMessage(); err != nil {
		...
		conn.Close()
		return &BrowserDialerError{Message: s}
	}

	return nil
}

type BrowserDialerError struct {
	Message string
}

func (e *BrowserDialerError) Error() string {
	return "browser dialer: " + e.Message
}
